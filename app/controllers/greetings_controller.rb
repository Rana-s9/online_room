class GreetingsController < ApplicationController
  ...
  def create
    ...
    if @greeting.save
        redirect_to room_greetings_path(@room), notice: t("flash.greeting.save")
    else
        flash.now[:alert] = t("flash.greeting.failed_save")
  ...
  def update
    ...
    if @greeting.update(greeting_params)
      redirect_to room_greetings_path(@room), notice: t("flash.greeting.update")
    else
      @greetings = @room.greetings.includes(:user).order(created_at: :desc)
      flash.now[:alert] = t("flash.greeting.failed_update")
      render :new, status: :unprocessable_entity
    end
  end
  ...
  def destroy
    ...
    if @greeting.destroy
      redirect_to room_greetings_path(@room), notice: t("flash.greeting.delete")
    else
      @greetings = @room.greetings.includes(:user).order(created_at: :desc)
      flash.now[:alert] = t("flash.greeting.failed_delete")
      render :new, status: :unprocessable_entity
    end
  end
